...
exports.PLAYER_CONFIG_PREIMAGE_SIZE = exports.PLAYER_CONFIG_HASH_VERSION = void 0;
exports.quantizeSpawn = quantizeSpawn;
exports.dequantizeSpawn = dequantizeSpawn;
exports.buildPlayerConfigPreimage = buildPlayerConfigPreimage;
exports.computePlayerConfigHash = computePlayerConfigHash;
exports.computePlayerConfigHashHex = computePlayerConfigHashHex;
exports.hexToUint8Array = hexToUint8Array;
exports.uint8ArrayToHex = uint8ArrayToHex;
// Current version of the preimage format
exports.PLAYER_CONFIG_HASH_VERSION = 1;
// Preimage size in bytes (fixed layout)
exports.PLAYER_CONFIG_PREIMAGE_SIZE = 53;
/**
 ...
 */
function quantizeSpawn(x, y, rot) {
 ...
 */
function dequantizeSpawn(x_q, y_q, rot_q) {
    const x = x_q / 32767;
    const y = y_q / 32767;
 ...
 */
function buildPlayerConfigPreimage(params) {
    const { roundId, tierId, playerPubkey, tpPreset, spawn, allocation, } = params;
    // Validate inputs
    if (playerPubkey.length !== 32) {
        throw new Error(`playerPubkey must be 32 bytes, got ${playerPubkey.length}`);
    }
    const preimage = new Uint8Array(exports.PLAYER_CONFIG_PREIMAGE_SIZE);
    const view = new DataView(preimage.buffer);
    let offset = 0;
    // version:u8 (1 byte)
    preimage[offset] = exports.PLAYER_CONFIG_HASH_VERSION;
    offset += 1;
    // round_id:u64_le (8 bytes)
    ...
    offset += 1;
    // Sanity check
    if (offset !== exports.PLAYER_CONFIG_PREIMAGE_SIZE) {
        throw new Error(`Preimage size mismatch: expected ${exports.PLAYER_CONFIG_PREIMAGE_SIZE}, got ${offset}`);
    }
    return preimage;
}
/**
 ...
 */
async function computePlayerConfigHash(params) {
 ...
 */
async function computePlayerConfigHashHex(params) {
